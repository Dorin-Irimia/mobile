...
function householdExpenseFromPayload(fields, files, clientId) {
  return {
    ...
    fromIncomeId: fields.fromIncomeId || null,
    customFields: parseCustomFields(fields.customFields),
    attachments: optimisticAttachments(files),
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
...
const useStore = create((set, get) => ({
  ...
  login: async (email, password) => {
    ...
    // Temporarily let API requests through during the upgrade-from-guest flow.
    // If login fails we restore the gate so the guest stays offline-only.
    if (wasGuest) setApiDisabled(false);
    try {
      ...
      // Login failed but we'd unlocked the API gate for the upgrade flow.
      // Restore it so the guest user doesn't end up with leaky API access.
      if (wasGuest) setApiDisabled(true);
  ...
  register: async (email, password, name, phone) => {
    ...
    try {
      ...
      // Mirror login(): if register failed during upgrade-from-guest, restore
      // the API gate so we don't leak API calls in guest mode.
      if (wasGuest) setApiDisabled(true);
      set({ error: msg, isLoading: false });
      return { success: false, error: msg };
    }
